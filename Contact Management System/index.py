...
def Database():
    ...
    conn = sqlite3.connect("contact1.db")
    cursor = conn.cursor()
    #creating REGISTRATION table
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS REGISTRATION (RID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, FNAME TEXT, LNAME TEXT, GENDER TEXT, ADDRESS TEXT,EMAILID TEXT, CONTACT TEXT)")
...
def DisplayForm():
    ...
    global tree
    global SEARCH
    global fname,lname,gender,address,emailid,contact
    SEARCH = StringVar()
    fname = StringVar()
    lname = StringVar()
    gender = StringVar()
    ...
    Label(LFrom, text="Email Id  ", font=("Arial", 12),bg="#15244C",fg="white").pack(side=TOP)
    Entry(LFrom,font=("Arial",10,"bold"),textvariable=emailid).pack(side=TOP, padx=10, fill=X)
    ...
    lbl_txtsearch = Label(LeftViewForm, text="Enter First Name to Search", font=('verdana', 10),bg="#0B4670")
    lbl_txtsearch.pack()
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "Name", "LastName", "Gender","Address","Email","Contact"),
...
def Update():
    ...
    if fname1=='' or lname1==''or gender1=='' or address1==''or emailid1=='' or contact1=='':
        ...
        #update query
        conn.execute('UPDATE REGISTRATION SET FNAME=?,LNAME=?,GENDER=?,ADDRESS=?, EmailId=?,CONTACT=? WHERE RID = ?',(fname1,lname1,gender1,address1,emailid1,contact1, selecteditem[0]))
        conn.commit()
...
def register():
    ...
    if fname1=='' or lname1==''or gender1=='' or address1==''or emailid1==''or contact1=='':
        ...
        #execute query
        conn.execute('INSERT INTO REGISTRATION (FNAME,LNAME,GENDER,ADDRESS,EMAILID,CONTACT) \
              VALUES (?,?,?,?,?,?)',(fname1,lname1,gender1,address1,emailid1,contact1));
        conn.commit()
